    ...
    local function getProfessionRanks(compName)
     ...
     for _, prof in ipairs(profList) do
        for _, spellID in ipairs(prof) do
            -- Prefer CA_IsSpellKnown for Ascension custom profession spells (e.g. Woodworking).
            local known = (type(CA_IsSpellKnown) == "function" and CA_IsSpellKnown(spellID))
                or (type(IsSpellKnown) == "function" and IsSpellKnown(spellID))
            if known and ((prof.Show and self.db[prof.Show]) or not prof.Show) then
